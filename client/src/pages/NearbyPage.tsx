...
export default function NearbyPage() {
  ...
  useEffect(() => setVisibleCount(12), [radius, onlineOnly]);
  ...
  return <div className="page-shell space-y-5">
    ...
    {users.length > 0 && <><motion.div layout className={view === 'grid' ? 'grid grid-cols-2 gap-3 md:grid-cols-3 lg:grid-cols-4' : 'space-y-3'}>{visibleUsers.map(item => <PersonCard key={getUserId(item.user)} item={item} compact={view === 'list'} onConnect={() => connectMutation.mutate(getUserId(item.user))} onChat={() => chatMutation.mutate(getUserId(item.user))} onReport={() => setReportModalUser(item)} />)}</motion.div><IonInfiniteScroll threshold="180px" disabled={visibleCount >= users.length} onIonInfinite={(event) => { setVisibleCount((count) => Math.min(count + 12, users.length)); event.target.complete(); }}><IonInfiniteScrollContent loadingSpinner="crescent" loadingText="Finding more people…" /></IonInfiniteScroll></>}
...
function PersonCard({ item, compact, onConnect, onChat, onReport }: { item: NearbyUserItem; compact: boolean; onConnect: () => void; onChat: () => void; onReport: () => void }) {
  ...
  return <motion.article layout whileHover={{ y: -3 }} className={`app-card group overflow-hidden ${compact ? 'flex items-center p-3' : ''}`}><div className={`relative overflow-hidden ${compact ? 'h-20 w-20 shrink-0 rounded-2xl' : 'h-44'}`}><img loading="lazy" decoding="async" src={item.user.avatarUrl} alt={item.user.displayName} className="h-full w-full object-cover transition duration-500 group-hover:scale-105" /><div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent" />{!compact && <span className="absolute bottom-3 left-3 rounded-full bg-white/90 px-2 py-1 text-[10px] font-bold text-gray-800"><IonIcon icon={location} /> {item.distanceKm ?? '?'} km</span>}<span className="absolute right-3 top-3 h-3 w-3 rounded-full border-2 border-white bg-emerald-500" /></div><div className={`min-w-0 ${compact ? 'flex flex-1 items-center gap-3 pl-3' : 'p-3.5'}`}><div className="min-w-0 flex-1"><div className="flex items-center gap-1"><h2 className="truncate text-sm font-extrabold">{item.user.displayName}</h2><span className="text-brand-600">●</span></div><p className="truncate text-[11px] text-gray-400">{item.mutualInterests.slice(0, 2).join(' · ') || `@${item.user.username}`}</p>{compact && <p className="mt-1 text-[10px] font-semibold text-brand-600">{item.distanceKm ?? '?'} km away</p>}</div><div className={`${compact ? 'flex' : 'mt-3 flex'} gap-2`}>{connected ? <button aria-label={`Message ${item.user.displayName}`} onClick={onChat} className="grid h-10 flex-1 place-items-center rounded-2xl bg-brand-600 px-3 text-white"><IonIcon icon={chatbubble} /></button> : item.connectionStatus === 'pending_sent' ? <span className="flex h-10 flex-1 items-center justify-center rounded-2xl bg-gray-100 px-2 text-[10px] font-bold text-gray-400 dark:bg-gray-800">Pending</span> : <button onClick={onConnect} className="flex h-10 flex-1 items-center justify-center gap-1 rounded-2xl bg-brand-600 px-3 text-[11px] font-bold text-white"><IonIcon icon={people} /> Connect</button>}<button aria-label={`Report ${item.user.displayName}`} onClick={onReport} className="grid h-10 w-10 place-items-center rounded-2xl bg-gray-100 text-gray-400 dark:bg-gray-800"><IonIcon icon={flagOutline} /></button></div></div></motion.article>;
}
