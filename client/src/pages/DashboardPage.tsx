      ...
      <motion.section {...fade} className="relative overflow-hidden rounded-[2rem] bg-[#20264c] p-5 text-white shadow-[0_24px_55px_-25px_rgba(40,48,105,.75)] sm:p-8">
        ...
        <div className="relative z-10 max-w-xl">
          ...
          <div className="mt-6 flex flex-wrap gap-2.5">
            <Button onClick={() => navigate('/nearby')} className="bg-[#20264c] text-white hover:bg-[#28305d] dark:bg-white dark:text-[#28305d] dark:hover:bg-indigo-50"><IonIcon icon={compass} /> Discover nearby<span className="sr-only">📍 Discover Nearby</span></Button>
        ...
        <div className="absolute bottom-6 right-7 hidden items-end lg:flex">
      ...
      <div className="grid gap-7 lg:grid-cols-[1.55fr_.85fr]">
        <div className="space-y-7">
          <section><SectionTitle eyebrow="Around you" title="People you may vibe with" to="/nearby" />
            {nearbyQuery.isPending ? <div className="flex gap-3 overflow-hidden">{[1,2,3].map(i => <Skeleton key={i} className="h-56 min-w-[175px] rounded-[1.5rem]" />)}</div> : nearby.length === 0 ? <CompactEmpty text="No one nearby yet. Update your location to start discovering." /> : <div className="scrollbar-none -mx-4 flex snap-x gap-3 overflow-x-auto px-4 pb-2 sm:mx-0 sm:px-0">{nearby.map(item => <motion.button whileTap={{ scale: .98 }} key={getUserId(item.user)} onClick={() => navigate('/nearby')} className="app-card min-w-[176px] snap-start overflow-hidden text-left"><div className="relative h-32"><Avatar src={item.user.avatarUrl} alt={item.user.displayName} seed={item.user.username || item.user.displayName} size="custom" shape="squircle" className="h-full w-full" imgClassName="rounded-none" /><div className="absolute inset-0 bg-gradient-to-t from-black/55 to-transparent" />{item.distanceKm !== null && <span className="absolute bottom-2 left-2 rounded-full bg-white/90 px-2 py-1 text-[10px] font-bold text-gray-800">{item.distanceKm} km</span>}</div><div className="p-3"><div className="flex items-center gap-1.5"><h3 className="truncate text-sm font-bold">{item.user.displayName}</h3><span className="h-2 w-2 shrink-0 rounded-full bg-emerald-500" /></div><p className="mt-0.5 truncate text-[11px] text-gray-400">{item.mutualInterests.slice(0,2).join(' · ') || `@${item.user.username}`}</p></div></motion.button>)}</div>}
          </section>

          <section><SectionTitle eyebrow="Keep talking" title="Recent conversations" to="/chat" />
            <div className="app-card divide-y divide-gray-100 overflow-hidden dark:divide-gray-800">{chats.length === 0 ? <CompactEmpty text="Your conversations will appear here." /> : chats.map(c => <button key={c._id} onClick={() => navigate('/chat', { state: { conversationId: c._id } })} className="flex min-h-[72px] w-full items-center gap-3 px-4 text-left transition hover:bg-gray-50 dark:hover:bg-gray-800/50"><Avatar src={c.recipient?.avatarUrl} alt={c.recipient?.displayName} seed={c.recipient?.username || c.recipient?.displayName} size="md" shape="squircle" showOnlineStatus isOnline /><div className="min-w-0 flex-1"><div className="flex justify-between gap-3"><h3 className="truncate text-sm font-bold">{c.recipient?.displayName}</h3><time className="text-[10px] text-gray-400">{c.lastMessageAt ? new Date(c.lastMessageAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : ''}</time></div><p className="mt-1 truncate text-xs text-gray-400">{c.lastMessage?.content || 'Start the conversation'}</p></div>{c.unreadCount > 0 && <span className="grid min-w-5 place-items-center rounded-full bg-brand-600 px-1.5 text-[10px] font-bold leading-5 text-white">{c.unreadCount}</span>}</button>)}</div>
          </section>
        </div>

        <section><SectionTitle eyebrow="New connections" title="Friend requests" to="/friends" /><div className="app-card p-3">{requests.length === 0 ? <CompactEmpty text="You're all caught up." /> : requests.map(req => <div key={req.id} className="flex items-center gap-3 rounded-2xl p-2.5 hover:bg-gray-50 dark:hover:bg-gray-800"><Avatar src={req.user.avatarUrl} alt={req.user.displayName} seed={req.user.username || req.user.displayName} size="md" shape="squircle" /><div className="min-w-0 flex-1"><h3 className="truncate text-sm font-bold">{req.user.displayName}</h3><p className="truncate text-[11px] text-gray-400">wants to connect</p></div><Button size="sm" onClick={() => navigate('/friends')}>Review</Button></div>)}</div>
